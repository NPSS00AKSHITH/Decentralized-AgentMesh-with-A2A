...
logger = setup_logging("dispatch-main")

skill_dispatch = AgentSkill(
    id="declare_incident",
    name="Incident Dispatch",
    description="Classify and route emergency incidents.",
...
card = AgentCard(
    name="dispatch-agent",
    description="Central incident router.",
    version="1.0.0",
    url=f"http://{os.getenv('SERVICE_HOST', 'dispatch-agent')}:{os.getenv('PORT', '9002')}/a2a",
    capabilities=AgentCapabilities(streaming=True),
    skills=[skill_dispatch],
    default_input_modes=["text/plain"],
    default_output_modes=["text/plain"]
)

handler = DefaultRequestHandler(
    agent_executor=DispatchExecutor(),
...
async def lifespan(app: FastAPI):
    ...
    await registry.register_service(
        ...
        tags=["dispatch", "coordination", "a2a"]
    )
    try:
        yield
    finally:
        # Shutdown
        await registry.deregister_service("dispatch-agent")
        await registry.close()

app = FastAPI(title="DDMS Dispatch Agent", lifespan=lifespan)
...
async def health_check():
    return {"status": "healthy", "role": "dispatch", "mode": "official-a2a-sdk"}
