...
from .fire_chief_agent import fire_chief_agent
from .tools import fire_map_lookup 
from lib.utils.logging_config import correlation_id_var
from lib.utils.communication import GlobalA2AClient

logger = logging.getLogger("fire-chief-executor")
a2a = GlobalA2AClient()

class FireChiefExecutor(AgentExecutor):
    def __init__(self):
        # Single shared runner instance for all requests
        self.runner = InMemoryRunner(fire_chief_agent)
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        try:
            ...
            # Fallback: If user_query looks like a Pydantic repr
        ...
        # Track if this is a delegation request that needs a telemetry response
        is_delegation = False
        ...
        try:
            ...
            try:
                ...
                if isinstance(data, dict):
                    ...
                    if msg_type == "HANDSHAKE_RESULT":
                        ...
                        # Check if this is a DELEGATION_REQUEST (agent-to-agent delegation)
                        if payload.get("type") == "DELEGATION_REQUEST":
                            is_delegation = True
                            delegation_source = source
                            delegation_request = payload.get("request", "")
                            final_query_text = (
                                f"DELEGATED TASK from {source}:\n"
                                f"{delegation_request}\n\n"
                                "INSTRUCTION: Process this request using your specialized tools. "
                                "In your response, LIST which tools you called (e.g., 'Tools used: fire_map_lookup, deploy_units'). "
                                "Provide detailed results from each tool."
                            )
                            logger.info(f"Processing Delegation from {source}: {delegation_request[:60]}...")
                        else:
                            # Standard handshake request
                            final_query_text = (
                                f"SYSTEM ALERT: Incoming Priority Request from {source}.\n"
                                f"DATA: {json.dumps(payload, indent=2)}\n"
                                "INSTRUCTION: Execute the necessary action immediately (e.g., deploy_units). "
                                "Then, you MUST call 'confirm_incident' to notify the sender."
                            )
            ...
            # If this was a delegation, send back a HANDSHAKE_RESULT with telemetry
            if is_delegation:
                result_payload = json.dumps({
                    ...
                    "agent": "fire-chief-agent"
                })
                
                # Send result back to source agent
                await a2a.send_message(
                    source_agent="fire-chief-agent",
                    target_agent=delegation_source,
                    message_text=result_payload,
                    correlation_id=correlation_id
                )
                logger.info(f"Sent delegation result to {delegation_source}: {len(final_text)} chars, {len(tools_called)} tools, {duration_ms}ms")
