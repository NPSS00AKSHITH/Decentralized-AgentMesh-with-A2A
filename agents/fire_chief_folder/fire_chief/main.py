import os
import uvicorn
from fastapi import FastAPI
from fastapi.middleware.cors import CORSMiddleware
from a2a.server.apps import A2AStarletteApplication
from a2a.server.request_handlers import DefaultRequestHandler
from a2a.types import AgentCard, AgentSkill, AgentCapabilities

from .executor import FireChiefExecutor
from lib.utils.logging_config import setup_logging
from lib.utils.postgres_task_store import PostgresTaskStore
from lib.utils.middleware import JWTMiddleware

logger = setup_logging("fire-chief-main")

skill_fire = AgentSkill(
    id="handle_fire_incident",
    name="Fire Incident Response",
    description="Tactical command for fire suppression.",
    input_modes=["text/plain"],
    output_modes=["text/plain"],
    tags=["fire", "rescue"]
)

card = AgentCard(
    name="fire-chief-agent",
    description="Fire response commander.",
    version="1.0.0",
    url=f"http://{os.getenv('SERVICE_HOST', 'fire-chief-agent')}:{os.getenv('PORT', '9003')}/a2a",
    capabilities=AgentCapabilities(streaming=True),
    skills=[skill_fire],
    default_input_modes=["text/plain"],
    default_output_modes=["text/plain"]
)

handler = DefaultRequestHandler(
    agent_executor=FireChiefExecutor(),
    task_store=PostgresTaskStore(dsn=os.getenv("DATABASE_URL"))
)

a2a_app = A2AStarletteApplication(agent_card=card, http_handler=handler).build()
a2a_app.add_middleware(JWTMiddleware, expected_audience=card.name)

from contextlib import asynccontextmanager
from lib.consul.registry import ConsulRegistry

@asynccontextmanager
async def lifespan(app: FastAPI):
    # Startup
    logger.info("Initializing Service...")
    
    # [FIX] Connect to DB at startup to prevent race conditions
    try:
        await handler.task_store.connect()
        logger.info("Task Store connected.")
    except Exception as e:
        logger.error(f"Failed to connect to Task Store: {e}")
    
    registry = ConsulRegistry()
    await registry.register_service(
        service_name="fire-chief-agent",
        port=int(os.getenv("PORT", 9003)),
        tags=["fire", "rescue", "a2a"]
    )
    try:
        yield
    finally:
        # Shutdown
        await registry.deregister_service("fire-chief-agent")
        await registry.close()
        
        # Close DB pool if possible
        if handler.task_store.pool:
            await handler.task_store.pool.close()

app = FastAPI(title="DDMS Fire Chief Agent", lifespan=lifespan)
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_credentials=True, allow_methods=["*"], allow_headers=["*"])
app.mount("/a2a", a2a_app)
app.mount("/.well-known", a2a_app)

@app.get("/health")
async def health_check():
    return {"status": "healthy", "role": "fire_chief", "mode": "official-a2a-sdk"}

@app.get("/.well-known/agent.json")
async def agent_card_endpoint():
    """Serve the agent card for A2A discovery."""
    return card.model_dump(by_alias=True, exclude_none=True)

if __name__ == "__main__":
    uvicorn.run(app, host="0.0.0.0", port=int(os.getenv("PORT", 9003)))
