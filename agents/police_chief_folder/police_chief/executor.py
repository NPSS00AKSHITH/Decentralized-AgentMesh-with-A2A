...
from .police_chief_agent import police_chief_agent
from lib.utils.logging_config import correlation_id_var
from lib.utils.communication import GlobalA2AClient

logger = logging.getLogger("police-chief-executor")
a2a = GlobalA2AClient()

class PoliceChiefExecutor(AgentExecutor):
    def __init__(self):
        # Single shared runner instance for all requests
        self.runner = InMemoryRunner(police_chief_agent)
                ...
                if isinstance(data, dict):
                    ...
                    if msg_type == "HANDSHAKE_RESULT":
                        ...
                        final_query_text = (
                            ...
                            "INSTRUCTION: Execute the necessary action immediately (e.g., deploy_swat, cordon_area). "
