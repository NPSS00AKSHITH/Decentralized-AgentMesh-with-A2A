...
from .civic_alert_agent import civic_alert_agent
from lib.utils.logging_config import correlation_id_var
from lib.utils.communication import GlobalA2AClient

logger = logging.getLogger("civic-alert-executor")
a2a = GlobalA2AClient()

class CivicAlertExecutor(AgentExecutor):
    def __init__(self):
        # Single shared runner instance for all requests
        self.runner = InMemoryRunner(civic_alert_agent)
                ...
                if isinstance(data, dict):
                    ...
                    if msg_type == "HANDSHAKE_RESULT":
                        ...
                        final_query_text = (
                            ...
                            "INSTRUCTION: Execute the necessary action immediately (e.g., broadcast_alert). "
                            "Then, you MUST call a confirmation tool if available to notify the sender."
                        )
