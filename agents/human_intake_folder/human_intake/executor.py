...
from .human_intake_agent import human_intake_agent
from lib.utils.logging_config import correlation_id_var
from lib.utils.communication import GlobalA2AClient

logger = logging.getLogger("human-intake-executor")
a2a = GlobalA2AClient()

class HumanIntakeAgentExecutor(AgentExecutor):
    def __init__(self):
        # Single shared runner instance for all requests
        self.runner = InMemoryRunner(human_intake_agent)
                ...
                if isinstance(data, dict):
                    ...
                    if msg_type == "HANDSHAKE_RESULT":
                        ...
                        final_query_text = (
                            ...
                            "INSTRUCTION: Process this request immediately (e.g., log_and_route_call). "
